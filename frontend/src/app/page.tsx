...
export default function Home() {
  ...
  return (
    <main
      ref={containerRef}
      className="relative min-h-screen bg-background overflow-hidden selection:bg-accent/30"
    >
      ...
      <section className="relative h-[100vh] flex flex-col items-center justify-center px-6 text-center">
        ...
        >
          ...
          >
            Since 2026, redefining the standard for high-jewelry craftsmanship.
            A sanctuary of GIA-certified diamonds and unparalleled artistry.
          </motion.p>
      ...
      <section className="py-maison px-6 md:px-12 lg:px-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          ...
          >
            <h3 className="text-[10px] uppercase font-bold text-primary tracking-[0.4em]">
              The Maison
            </h3>
            <h2 className="text-5xl md:text-7xl uppercase tracking-tighter font-serif leading-[0.9]">
              Craftsmanship <br /> & <span className="italic">Heritage</span>
            </h2>
            <p className="text-muted-foreground font-light leading-relaxed max-w-md">
              Our artisans combine century-old techniques with pioneering
              digital innovation to breathe life into every stone. Every piece
              is a testament to the pursuit of absolute perfection.
            </p>
            <Link
              href="/maison"
              className="inline-block text-[10px] uppercase tracking-[0.2em] font-bold border-b border-foreground pb-2 hover:border-primary transition-all duration-300"
            >
              Discover Our Story
            </Link>
          </motion.div>
      ...
      <section className="bg-muted py-maison">
        <div className="container mx-auto px-6 mb-24 text-center">
          <h2 className="text-4xl md:text-6xl uppercase tracking-tighter font-serif font-light">
            The Atelier <span className="italic">Edit</span>
          </h2>
        </div>
        <div className="flex flex-col md:flex-row h-auto gap-4 px-4">
          <div className="flex-1 aspect-[16/10] bg-zinc-200 relative group overflow-hidden">
            ...
            <div className="absolute bottom-12 left-12">
              <h4 className="text-2xl font-serif text-background uppercase tracking-tight">
                Celestial High Jewelry
              </h4>
              <Link
                href="/high-jewelry"
                className="text-[10px] uppercase tracking-widest text-background border-b border-background mt-4 block w-fit"
              >
                View Story
              </Link>
            </div>
      ...
      <section className="py-maison px-6 md:px-24">
        <div className="max-w-6xl mx-auto border border-border p-12 md:p-24 flex flex-col items-center text-center space-y-12 bg-[#fafafa]">
          <div className="text-[10px] uppercase font-bold text-primary tracking-[0.4em]">
            B2B Terminal Access
          </div>
          <h2 className="text-4xl md:text-7xl uppercase tracking-tighter font-serif font-light leading-[0.85]">
            Global Market <br /> <span className="italic">Intelligence</span>
          </h2>
          <p className="max-w-2xl text-muted-foreground font-light leading-relaxed">
            For institutional partners only. Gain real-time access to our global
            GIA-certified inventory, wholesale pricing matrix, and specialized
            memo procurement services.
          </p>
          <div className="flex gap-8">
            <Link
              href="/diamonds"
              className="px-12 py-5 bg-foreground text-background text-[10px] uppercase font-bold tracking-widest hover:opacity-90 transition-all"
            >
              Access Inventory
            </Link>
            <Link
              href="/partner/apply"
              className="px-12 py-5 border border-border text-[10px] uppercase font-bold tracking-widest hover:border-foreground transition-all"
            >
              Partner Application
            </Link>
          </div>
      ...
      <section className="py-maison px-6 md:px-12 text-center border-t border-border mt-maison">
        <div className="max-w-xl mx-auto space-y-8">
          <h3 className="text-[10px] uppercase font-bold tracking-[0.3em] text-primary">
            The Maison Journal
          </h3>
          <h4 className="text-3xl font-serif tracking-tighter uppercase font-light">
            Elegance in Your <span className="italic">Inbox</span>
          </h4>
          <div className="relative group">
            <input
              type="email"
              placeholder="Enter Email Address"
              className="w-full py-6 pr-24 bg-transparent border-b border-border text-[10px] uppercase tracking-[0.2em] font-semibold focus:outline-none focus:border-foreground transition-all"
            />
            <button className="absolute right-0 top-1/2 -translate-y-1/2 text-[10px] uppercase font-bold tracking-widest hover:text-primary transition-colors">
              Join Now
            </button>
          </div>
          <p className="text-[8px] uppercase tracking-widest text-muted-foreground">
            Receive exclusive invitations, rare stock alerts, and high-jewelry
            editorials.
          </p>
        </div>
      </section>
