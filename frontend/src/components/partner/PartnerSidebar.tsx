...
export default function PartnerSidebar({ isOpen, setIsOpen }: PartnerSidebarProps) {
  const pathname = usePathname();
  const { logout } = useAuthStore();

  const navItems = [
    { name: 'Dashboard', href: '/partner/dashboard', icon: LayoutDashboard },
    { name: 'My Diamonds', href: '/partner/diamonds', icon: Gem },
    { name: 'My Orders', href: '/partner/orders', icon: ShoppingCart },
    { name: 'Profile', href: '/partner/profile', icon: UserIcon },
  ];
    ...
    <>
      ...
      <aside
        className={clsx(
          ...
          "bg-gradient-to-b from-[#0c1220] via-[#0f1628] to-[#0a0f1a] text-zinc-300 shadow-2xl",
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4 border-b border-white/[0.06]">
          <Link href="/partner/dashboard" className="flex items-center gap-2.5 min-w-0">
            <div className="flex-shrink-0 h-8 w-8 rounded-lg bg-gradient-to-br from-teal-400 to-emerald-600 flex items-center justify-center shadow-lg shadow-teal-500/20">
              <Gem className="h-4 w-4 text-white" />
            </div>
            {isOpen && (
              <span className="text-lg font-bold text-white tracking-tight truncate">
                Partner<span className="text-teal-400">Hub</span>
              </span>
            )}
          </Link>
                ...
                >
                  <Icon
                    className={clsx(
                      ...
                      isActive ? "text-teal-400" : "group-hover:text-teal-400/60"
                    )}
                  />
