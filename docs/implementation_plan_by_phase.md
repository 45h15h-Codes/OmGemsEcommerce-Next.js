# Role-Based Backend & Dashboard — Phased Implementation Plan

> Reorganized from the [original plan](file:///c:/Users/ASUS/.gemini/antigravity/brain/27cfda93-7c29-40d4-ac9c-081f5d12d90b/implementation_plan.md.resolved) into **6 balanced phases** ordered by dependency and workload.
...
flowchart TD
    ...
    Admin[Admin Panel\n/admin]
    Vendor[Partner / Vendor Panel\n/partner/dashboard]
    Wholesale[Wholesale Dashboard\n/wholesale/dashboard]
    Retail[Retail Account\n/account]

    %% Flow
    User --> Auth
    Auth -- Role: Super Admin --> SuperAdmin
    Auth -- Role: Admin --> Admin
    Auth -- Role: Partner --> Vendor
    Auth -- Role: Wholesale Buyer --> Wholesale
    Auth -- Role: Retail Customer --> Retail

    %% Super Admin Capabilities
    SuperAdmin -->|Complete Control| SA1[Full CMS / Page Management]
    SuperAdmin --> SA2[Global Jewelry/Diamond Listings]
    SuperAdmin --> SA3[System Config & Role Assignment]

    %% Admin Capabilities
    Admin -.->|Restricted Access| SA1
    Admin -.->|Restricted Access| SA2
    Admin --> Admin1[Only Assigned Modules]

    %% Partner Capabilities
    Vendor --> V1[Upload/Manage Own Diamonds]
    Vendor --> V2[View Partner Sales/Invoices]
    Vendor --> V3[Respond to Admin Queries]

    %% Wholesale Buyers
    Wholesale --> W1[Bulk Order Interface]
    Wholesale --> W2[B2B Restricted Pricing Access]
    Wholesale --> W3[Wire/Net-30 Invoices & History]
...
flowchart LR
    ...
    style P6 fill:#fafafa,stroke:#424242
```

> [!NOTE]
> Phases 3, 4, and 5 can run **in parallel** once Phase 2 is complete. Phase 6 waits for all of them.

---

## Phase 1 — Backend Foundation (Permissions & API)

**Goal:** Establish the granular Spatie permission system and make the API permission-aware.
...
- Define **granular permissions** and map them to roles:
  ...
  Admin        → Configurable subset (e.g., manage_orders, manage_diamonds, view_reports)
  Partner      → manage_own_diamonds, view_own_orders, update_own_profile
  Wholesale    → place_bulk_orders, view_b2b_pricing, manage_own_profile
  Retail       → view_own_orders, manage_wishlist, manage_own_profile
  ```
...
- Accessing a protected endpoint without the right permission returns `403`

---

## Phase 2 — Frontend Auth & Role-Based Routing

**Goal:** Build the unified login page and set up middleware that routes each role to their correct dashboard.
...
- Next.js edge middleware for route protection:
  ...
  /wholesale/*    → requires role Wholesale Buyer
...
## Phase 3 — Admin Panels (Super Admin & Admin)

**Goal:** Build the shared `/admin` panel shell with a dynamic, permission-aware sidebar.
...
- Navigation items:
  ...
  |--------|-------------------|-------------|-------|
  | Dashboard | — | ✅ | ✅ |
  | Diamonds | `manage_diamonds` | ✅ | Conditional |
  | Products | `manage_products` | ✅ | Conditional |
  | Orders | `manage_orders` | ✅ | Conditional |
  | Users & Roles | `manage_users` | ✅ | ❌ (Super Admin only) |
...
- User role assignment works end-to-end

---

## Phase 4 — Partner (Vendor) Portal

**Goal:** Build the dedicated partner workspace at `/partner/*` for diamond suppliers.
...
- Partner dashboard:
  ...
  - Revenue summary (their stones only)
  - Recent activity
...
- CSV diamond upload works end-to-end

---

## Phase 5 — Buyer Portals (Wholesale & Retail)

**Goal:** Build the Wholesale B2B dashboard and the Retail customer account pages.
...
- Data is strictly scoped to the logged-in user

---

## Phase 6 — Hardening, Security & QA

**Goal:** Audit for privilege escalation, add acceptance tests, and polish the UI.
...
- **IDOR Prevention:** Verify every endpoint that accepts an `{id}` parameter checks ownership/role
- **Privilege Escalation:** Test that forcefully navigating to other role's routes returns 403
- **Token Security:** Ensure tokens are scoped and expired tokens are rejected
- **Input Validation:** Audit all form inputs for XSS/injection
...
> 4. For Phase 3 (Admin),would you prefer a **pre-built admin UI kit** (like shadcn/ui) or a **fully custom design**?
